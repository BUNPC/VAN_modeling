...
% 06/05/2018: -added higher resolution factor Ndivide by X. Cheng
%             -default species is set to be 3 (mouse)

function MCproton_Hybrid_FineResolution_noGX_Flow(B0,phi_angle,omega_angle,TE,Gx,ROIx,ROIy,ROIz,file_name,iFrame,volume_and_mesh_file,vol_iFrame,ref_file,ref_iFrame,vol_ref_iFrame,VesselDiPercent,savefolder,cmro2,FlowChange)
...
Hvox=1/Ndivide; %in um (isotropic because the Fourier kernel assumes a sphere!)
...
if ~isfield(im2,'species')
    species=3; %set to mouse pO2-sO2 curve otherwise specified
else
    species=im2.species;
end
if ~exist('Vtime')
...
%mask dimension
nx=ceil(im2.nX.*im2.Hvox(1)*Ndivide);
ny=ceil(im2.nY.*im2.Hvox(2)*Ndivide);
nz=ceil(im2.nZ.*im2.Hvox(3)*Ndivide);
...
im2.nodeDiam=im2.nodeDiam*sqrt(1+VolumeChange/100); % changethe volume
im2.segDiam=im2.segDiam*sqrt(1+VolumeChange/100);
...
for iiE=1:length(lstEdges)
    ...
    if (nodeMasked(i1)==0 || nodeMasked(i2)==0) %% if this node is not masked
        if ~im2.maskUseSegDiam
            r1 = im2.nodeDiam(i1)/2.*RelVol(im2.nodeSegN(i1)).^(1/2)*Ndivide;
            r1 = min(max(r1,1),maxR);
            r2 = im2.nodeDiam(i2)/2.*RelVol(im2.nodeSegN(i2)).^(1/2)*Ndivide;
            r2 = min(max(r2,1),maxR);
            
        else
            foo = max( im2.segDiam(im2.nodeSegN(i1))*Ndivide.*RelVol(im2.nodeSegN(i1)).^(1/2) , im2.segDiam(im2.nodeSegN(i2))*Ndivide.*RelVol(im2.nodeSegN(i2)).^(1/2) )/2;
...
%                 end
    ...
    if (nodeMasked_ref(i1)==0 || nodeMasked_ref(i2)==0)
        if ~im2.maskUseSegDiam
            r1 = im2.nodeDiam(i1)/2*Ndivide.*RelVol_ref(im2.nodeSegN(i1)).^(1/2);
            r1 = min(max(r1,1),maxR);
            r2 = im2.nodeDiam(i2)/2*Ndivide.*RelVol_ref(im2.nodeSegN(i2)).^(1/2);
            r2 = min(max(r2,1),maxR);
            
        else
            foo = max( im2.segDiam(im2.nodeSegN(i1))*Ndivide.*RelVol_ref(im2.nodeSegN(i1)).^(1/2) , im2.segDiam(im2.nodeSegN(i2))*Ndivide.*RelVol_ref(im2.nodeSegN(i2)).^(1/2) )/2;
            foo = min(max(foo,1),maxR);
            r1 = foo;
            r2 = foo;
...
%Spin Echo params
half_echo_index=floor(TE/(2*dt))+1;
echo_index=min(floor(TE/dt),ntime_step);
start1_se_index=floor(TE/(4*dt)-TE/(8*dt))+1;
stop1_se_index=floor(TE/(4*dt)+TE/(8*dt));
start2_se_index=floor(TE/(dt)-TE/(4*dt))+1;
stop2_se_index=floor(TE/dt+TE/(4*dt));

%Gradient Echo params
start_ge_index=floor(TE/(3*dt))+1;
flip_ge_index=floor(2*TE/(3*dt))+1;
stop_ge_index=floor(4*TE/(3*dt))+1;
...
mkdir(savefolder,'FineResolution')

save([savefolder,'/FineResolution/MCBOLD_B0_',num2str(B0),'VesselDiPercent_',num2str(VesselDiPercent),'_cmro2_',num2str(cmro2),'.mat'],'tMCproton','TE','signal_GE','signal_SE','signal_GE_IV','signal_SE_IV','signal_GE_EV','signal_SE_EV',...
