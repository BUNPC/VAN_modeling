function VAN_advection_setup(CMRO2_baseline,duration_simulation,savefolder,VesselDiPercent,CMRO2_ini)
...
im2.po2t = 10;      % initial conditions for po2 everywhre

%CMRO2 (trace shape is average of active dilation)
im2.OCo_umol_per_mL_per_min = CMRO2_baseline; % umol / cm^3 / min

%im2.OCo_umol_per_mL_per_min = CMRO2_baseline*1000; % umol / cm^3 / min
...
elseif VesselDiPercent ~=0
    
    load([savefolder,'advection_',num2str(CMRO2_ini),'_',num2str(18*1000),'_',num2str(0),'ms.mat']);
    for iSim=1:duration_simulation
        tStart=iSim-1;
        c1=c(:,end);
        cg1=cg(:,end);
